...
def influence(kshell, brink):
    ...
    G = nx.read_gml('./dataset/polbooks.gml', label='id')
...
def EQ(result):
    with open(r"/home/zm/桌面/copra/polbooks_detection.txt", 'a') as f:
        for label, nodes in result.items():
            str_nodes = [str(n) for n in nodes]
            content = " ".join(str_nodes)
            f.write(content+"\n")
    cmd1 = "cd /home/zm/桌面/copra"
    cmd2 = "java -cp copra.jar ModOverlap polbooks.txt polbooks_detection.txt"
    cmd = cmd1+"&&"+cmd2
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
    output, _ = p.communicate()
    with open(r"/home/zm/桌面/copra/polbooks_detection.txt", 'w') as f:
        f.truncate()
...
if __name__ == '__main__':
    ...
    for r in R:
        ...
        for i in range(100):
            ...
            result = find_communities(G, 80, r, IN)
            print(result)
            G = nx.read_gml(r'./dataset/polbooks.gml', label='id')
            brink_node_label = {}
            ...
            for brink_node in brink:
                ...
                max_neighbors_nodes = [node for node, IN in neighbors_influence.items() if IN == max_neighbor_IN]
                max_neighbors_label = [node_label[node] for node in max_neighbors_nodes if node_label.get(node, False)]
