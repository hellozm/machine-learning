...
def EQ(result):
    with open(r"/home/zm/桌面/copra/football_detection.txt", 'a') as f:
        for label, nodes in result.items():
            str_nodes = [str(n) for n in nodes]
            content = " ".join(str_nodes)
            f.write(content+"\n")
    cmd1 = "cd /home/zm/桌面/copra"
    cmd2 = "java -cp copra.jar ModOverlap football.txt football_detection.txt"
    cmd = cmd1+"&&"+cmd2
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
    output, _ = p.communicate()
    with open(r"/home/zm/桌面/copra/football_detection.txt", 'w') as f:
        f.truncate()
...
if __name__ == '__main__':
    ...
    for r in R:
        ...
        final.append(sum(EQs) / len(EQs))
        mean_com.append(sum(com) / len(com))
    print(final)
    print(mean_com)
